...
bool CSFTPSession::Connect(const CStdString &host, const CStdString &port, const CStdString &username, const CStdString &password)
{
  ...
  if (password.empty())
  {
    if (!PerformKeyAuthentication(username))
    {
      CLog::Log(LOGINFO, "SFTPSession::Connect: Key authentication failed and no password given.");
      return false;
    }
  }
  else
  {
    if (!PerformPasswordAuthentication(username, password))
    {
...
bool
CSFTPSession::PerformKeyAuthentication(const CStdString& username)
{
  const char* userauthlist = libssh2_userauth_list(m_session, username.c_str(),
                                                   username.size());

  if (strstr(userauthlist, "publickey") == NULL)
  {
    CLog::Log(LOGINFO, "SFTPSession::Connect: Key authentication not available");
    return false;
  }
  ...
  if (!agent)
  {
    CLog::Log(LOGERROR, "SFTPSession::Connect: agent init failed");
    return false;
  }
  int rc = libssh2_agent_connect(agent);
  if (rc)
  {
    CLog::Log(LOGINFO, "SFTPSession::Connect: Couldn't connect to agent: %s",
              GetErrorString(rc));
  ...
  if (rc)
  {
    CLog::Log(LOGERROR, "SFTPSession::Connect: List identities failed: %s",
              GetErrorString(rc));
  ...
  while (true)
  {
    ...
    if (rc == 1)
    {
      CLog::Log(LOGINFO, "SFTPSession::Connect: Found no matching keyfile");
      break;
    }
    if (rc < 1)
    {
      CLog::Log(LOGERROR, "SFTPSession::Connect: Get identity failed: %s",
                GetErrorString(rc));
      break;
    }
    ...
    if (rc)
    {
      CLog::Log(LOGINFO, "SFTPSession::Connect: Authentication for user %s "
                "with key %s failed: %s", username.c_str(), identity->comment,
                GetErrorString(rc));
    }
    else
    {
      CLog::Log(LOGINFO, "SFTPSession::Connect: Successfull authentication for "
                "user %s with key %s", username.c_str(), identity->comment);
      authenticated = true;
      break;
    }
...
bool
CSFTPSession::PerformPasswordAuthentication(const CStdString& username,
                                            const CStdString& password)
{
  int rc = libssh2_userauth_password_ex(m_session, username.c_str(),
